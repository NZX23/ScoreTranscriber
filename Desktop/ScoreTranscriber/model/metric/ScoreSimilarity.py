...
def scoreSimilarity(estScore, gtScore):
    ...
    def compareSets(aSet, bSet):
        ...
        def referClef(noteObj): # added
            return noteObj.getContextByClass('Clef').name if noteObj.getContextByClass('Clef') is not None else music21.clef.bestClef(music21.stream.Stream([noteObj])).name #''

        def referTimeSig(noteObj): # added
            return noteObj.getContextByClass(music21.meter.TimeSignature).numerator / noteObj.getContextByClass(music21.meter.TimeSignature).denominator \
                    if noteObj.getContextByClass(music21.meter.TimeSignature) is not None else ''

        def referKeySig(noteObj): # added
            keyObj = (noteObj.getContextByClass('Key') or noteObj.getContextByClass('KeySignature') or noteObj.getContextByClass(music21.key.KeySignature))
                    ...
                    if isinstance(bObj[1], music21.note.Note) and bObj[1].pitch == obj[1].pitch:
                        if bObj[0] != obj[0]:
                            ...
                            if referTimeSig(bObj[1]) != referTimeSig(obj[1]): # added
                                ...
                                print(referTimeSig(bObj[1]), referTimeSig(obj[1]), bObj[1])
