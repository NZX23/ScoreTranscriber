...
def generate_midi_seg(source, perf_midi_path, xml_segments):
    ...
    for seg_id, filex in enumerate(xml_segments):
        ...
        # print(' OLD first_seg_note: ' + str(first_seg_note)) 
        # print(' OLD last_seg_note: ' + str(last_seg_note)) 
        if int(first_seg_note[1:].split('-')[0]) - boundary_offset > 0:
            first_seg_note = 'n' + str(int(first_seg_note[1:].split('-')[0]) - boundary_offset) + ''.join(first_seg_note.split('-')[1:])
        ...
        last_seg_note = 'n' + str(int(last_seg_note[1:].split('-')[0]) + boundary_offset) + ''.join(last_seg_note.split('-')[1:])
        ...
        #                 print('mismatch of repeat_id ' + str(value))
                ...
                if not firstSegMatched: # first_seg is not matched
...
if __name__ == "__main__": 
    ...
    tuples = [(source, midi_file_path) for source, midi_file_path in zip(metadata.source.values, metadata.midi_perfm.values)]

    with Pool() as pool: # use all CPUs (16 in this case)
        # execute tasks in order
        for result in pool.imap(multiprocess_midi_seg, tuples):
            print(f'Got result: {result}', flush=True)
            all_problem_files.append(result)

    print('DONE!!')

    with open("problem_files_midi_seg.txt", 'w') as f:
        for line in all_problem_files:
            f.write(f"{line}\n")