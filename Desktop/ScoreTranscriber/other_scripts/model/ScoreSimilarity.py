...
# THIS IS THE NEW ONE
# from ScoreTransformer Github

# 3 categories according to https://archives.ismir.net/ismir2021/latebreaking/000032.pdf 
# note preservation: note (3,4,5,6) and rest (10,11,12)
# note segregation: staff assignment (13) and voice separation (14)
# score attributes: clef, key signature and time signature (0,1,2)

class ScoreErrors(IntEnum):
    # sets index to index into errors[]
    Clef = 0
    KeySignature = 1
    TimeSignature = 2
    NoteDeletion = 3
...
def scoreSimilarity(estScore, gtScore):
    ...
    def compareSets(aSet, bSet):
        ...
        def referClef(noteObj): # added
            return noteObj.getContextByClass('Clef', getElementMethod=ElementSearch.ALL).name if noteObj.getContextByClass('Clef', getElementMethod=ElementSearch.ALL) is not None else music21.clef.bestClef(music21.stream.Stream([noteObj])).name #''

        def referTimeSig(noteObj): # added
            numerator = noteObj.getContextByClass(music21.meter.TimeSignature)
            # print(numerator)
            return noteObj.getContextByClass(music21.meter.TimeSignature, getElementMethod=ElementSearch.ALL).numerator / noteObj.getContextByClass(music21.meter.TimeSignature, getElementMethod=ElementSearch.ALL).denominator \
                    if noteObj.getContextByClass(music21.meter.TimeSignature, getElementMethod=ElementSearch.ALL) is not None else ''

        def referKeySig(noteObj): # added
            keyObj = (noteObj.getContextByClass('Key', getElementMethod=ElementSearch.ALL) or noteObj.getContextByClass('KeySignature', getElementMethod=ElementSearch.ALL) or noteObj.getContextByClass(music21.key.KeySignature, getElementMethod=ElementSearch.ALL))
                    ...
                    if isinstance(bObj[1], music21.note.Note) and bObj[1].pitch == obj[1].pitch:
                        if bObj[0] != obj[0]:
                            ...
                            if referTimeSig(bObj[1]) != referTimeSig(obj[1]): # added
                                ...
                                # print(referTimeSig(bObj[1]), referTimeSig(obj[1]), bObj[1], obj[1])
