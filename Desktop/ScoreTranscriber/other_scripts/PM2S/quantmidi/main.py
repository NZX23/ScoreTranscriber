...
def evaluate(args):
    ...
    for i, row in metadata.iterrows():
        ...
        beats_targ = annotations['beats'] # groundtruth from MIDI File
        downbeats_targ = annotations['downbeats'] # groundtruth from MIDI File
        ...
        if args.model_type == 'baseline':
            ...
            print(midi_score_file) #outputs/R_1_proposed_test.mid
            ...
            # save note values and hands as .pkl files/prediction
            ...
            # un-comment below paragraph if want to evaluate on non-post-processed midi
            # midi_score_file = 'outputs_post/post_'+ file
            # print(midi_score_file)
  
            # debugging - trying to print notes out
            # import pretty_midi as pm
            # file_name = row['midi_perfm']
            # print(file_name)
            # midi_targ = pm.PrettyMIDI(str(Path('/media/hdd1/data/' + file_name)))
    ...
    if args.model_type == 'proposed':
        print('\n ======== MV2H evaluation =========')
        print('THESE RESULTS HAVE BEEN FILTERED >0.9!! DO NOT TAKE THEM')
...
def evaluate_mv2h(args):
    ...
    for i, row in metadata.iterrows():
        ...
        # midi_pred_file = str(Path('outputs_post', 'post_'+row['performance_id']+'_proposed_test.mid'))
...
def get_test_performance_midis(args):
    ...
    for i, row in metadata.iterrows():
        ...
        path = '/media/hdd1/data/' + row['midi_perfm']
        midi_data_score = pm.PrettyMIDI(path)
...
def mv2h_evaluation(target_midi_file, output_midi_file, MV2H_path, timeout=20.):
        try:
            output = check_output(['sh', 'evaluate_midi_mv2h.sh', 
                                    '/media/hdd1/data/' + target_midi_file, output_midi_file, MV2H_path], 
                                    timeout=timeout)
