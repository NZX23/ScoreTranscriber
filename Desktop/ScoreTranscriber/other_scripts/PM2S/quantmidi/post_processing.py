...
# from data.constants import resolution, tolerance
...
def generate_MIDI_score(note_sequence, annotations, row_id, output_file):
    ...
    for ni in range(length):
        ...
        master_onset_ticks.append(onset_ticks/ticks_per_beat)

    # save master_onset_ticks as .pkl
                
    # return master_onset_ticks
        ...
        if onset_ticks - prev_onset_ticks >= 10:
            ...
            # Scale the tempo down to fit within the valid range
            tempo_scale = max(0, min(16777215 / tempo, 1))
            tempo = int(tempo * tempo_scale)
