...
# SOURCE: THIS FILE IS FROM PM2S AUTHOR'S CODE
...
class FeaturePreparation():
    ...
    def prepare_metadata(self):
        ...
        # =========== get ASAP metadata ===========
        ASAP_metadata = pd.read_csv(str(Path(self.ASAP, 'metadata.csv')))
        ...
        for _, row in ACPAS_metadata.iterrows():
            ...
            if row['source'] == 'ASAP':
                # lookup the midi file insided ASAP_metadata and get the xml_path from 'xml_score' column
                midi_perf_file = midi_perfm.replace(self.ASAP,'')[1:] # remove ASAP folder path from MIDI perf path in order to match, remove '/' at first char
                xml_path = ASAP_metadata.loc[ASAP_metadata['midi_performance'] == midi_perf_file, 'xml_score'].values[0]
                ...
                metadata = metadata.append({
                    ...
                    'xml_file':  xml_path, # need to + {ASAP}?
                }, ignore_index=True)
    ...
    def load_metadata(self):
        ...
        self.metadata = pd.read_csv(str(Path(self.feature_folder, 'combined_metadata.csv')))
    ...
    def print_statistics(self):
        ...
        pieces_train = set(self.metadata[self.metadata['split'] == 'train']['xml_file'].unique())
        pieces_valid = set(self.metadata[self.metadata['split'] == 'valid']['xml_file'].unique())
        pieces_test = set(self.metadata[self.metadata['split'] == 'test']['xml_file'].unique())
        ...
        for _, row in self.metadata.iterrows():
            ...
            # if row['source'] == 'ASAP' or row['source'] == 'Vienna' or row['source'] == 'Batik':
            #     continue
        ...
        path_final = '../ATEPP/Franz_Liszt/10_Harmonies_poétiques_et_religieuses,_S.173/No._7._Funerailles/00305.mid'

        path_displayed = '../ATEPP/Franz_Liszt/10_Harmonies_poétiques_et_religieuses,_S.173/No._7._Funerailles/00305.mid'

        print(path_final == path_displayed)
    ...
    def prepare_features(self):
        # prepares the pickle file consisting of (beats, downbeats, key signatures, time signatures) from MIDI file for every MIDI segment in the MIDI_seg directory

        MIDI_seg_directory = '2bar_midi_segments/'
        filenames = next(walk(MIDI_seg_directory), (None, None, []))[2] 
        ...
        def prepare_one_feature(row):
            ...
            note_sequence = DataUtils.get_note_sequence_from_midi('2bar_midi_segments/' + row)
            
            pickle_filename = row.replace('.mid', '.pkl')
            pickle.dump((note_sequence), open('2bar_features_pickle/' + pickle_filename, 'wb'))
...
if __name__ == '__main__':
    # usage:
    # python feature_preparation.py --dataset_folder ASAP/asap-dataset-main /A-MAPS_1.2 /CPM /ACPAS --feature_folder /features --workers -2
    ...
    # featprep.prepare_metadata() # only need to run this line once, to generate metadata.csv

    # featprep.load_metadata()
    # featprep.print_statistics()
    print('OINKKKKK')

    featprep.prepare_features() # run this function to generate all the .pkl files referenced in metadata.csv